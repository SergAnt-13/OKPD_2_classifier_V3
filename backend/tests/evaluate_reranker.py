# backend/tests/evaluate_reranker.py
# Purpose: Сравнение Dense only vs Dense + Reranker на стратифицированной выборке.
import sys
...
# Стратификация по группам head/mid/tail
...
# Убираем unseen из test
train_codes = set(train_df["true_code"])
test_df = test_df[test_df["true_code"].isin(train_codes)]

print(f"Test: {len(test_df)} (head={len(test_df[test_df['group']=='head'])}, mid={len(test_df[test_df['group']=='mid'])}, tail={len(test_df[test_df['group']=='tail'])})")
...
def evaluate_with_reranker(queries_df):
    ...
    ndcg_sum = 0.0
    for _, row in tqdm(queries_df.iterrows(), total=len(queries_df), desc="With Reranker"):
...
print("Замер Dense + Reranker...")
res_rerank, ndcg_rerank = evaluate_with_reranker(test_df)

print("\n" + "="*70)
print("СРАВНЕНИЕ НА СТРАТИФИЦИРОВАННОМ ТЕСТЕ")
print("="*70)
...
print(f"{'Dense + Reranker':<25} {res_rerank[1]:<8.4f} {res_rerank[3]:<8.4f} {res_rerank[5]:<8.4f} {res_rerank[10]:<8.4f} {ndcg_rerank:<8.4f}")